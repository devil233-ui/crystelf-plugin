...
export function getCacheLimit(url, config = {}) {
  return url === "global_dedupe"
    ? config.maxRssDedupe ?? 500
    : config.maxFeed ?? 10;
}

export function mergeDedupeHistory(data, maxCache = 500) {
  const links = [];
  const entries = Object.entries(data || {});
  const globalEntry = entries.find(([ url ]) => url === "global_dedupe");
  const orderedEntries = globalEntry
    ? [ globalEntry, ...entries.filter(([ url ]) => url !== "global_dedupe") ]
    : entries;
  ...
  for (const [ , value ] of orderedEntries) {
    ...
    for (const link of items) {
      ...
      if (links.length >= maxCache) return links;
    }
  }
  return links;
}
...
const rssCache = {
  ...
  async set(url, latestLink) {
    ...
    const maxCache = getCacheLimit(url, config);
    cached = [ latestLink, ...cached.filter((l) => l !== latestLink) ].slice(0, maxCache);
  ...
  async loadLocalToRedis() {
    ...
    try {
      ...
      const maxCache = getCacheLimit("global_dedupe", configControl.get("config") || {});
      const mergedHistory = mergeDedupeHistory(data, maxCache);
      const previousGlobal = Array.isArray(data.global_dedupe) ? data.global_dedupe : [];
      if (mergedHistory.length > 0) {
        await redis.set(this.urlToKey("global_dedupe"), JSON.stringify(mergedHistory));
      }
      if (JSON.stringify(previousGlobal) !== JSON.stringify(mergedHistory)) {
        data.global_dedupe = mergedHistory;
